...
//	THTMLStyleSheet = class;
	...
	TTagState = (tsNone, tsBegin, tsEnd, tsPropertyName, tsSpace,
		tsEqual, tsPropertyValue);
	...
	THTMLAlign = (haLeft, haRight, haTop, haTextTop, haMiddle,
  ...
  private
	...
	TStyleSheetElement = (
		...
		seLineHeight,
		//Border
		seMarginTop, seMarginRight, seMarginBottom, seMarginLeft,
		sePaddingTop, sePaddingRight, sePaddingBottom, sePaddingLeft,

		seWidth, seHeight, seClear, seFloat,
		seBorder, seBorderStyle, seBorderColor, seBorderWidth,
		seBorderTop, seBorderTopWidth, seBorderTopStyle, seBorderTopColor,
		seBorderRight, seBorderRightWidth, seBorderRightStyle, seBorderRightColor,
		seBorderBottom, seBorderBottomWidth, seBorderBottomStyle, seBorderBottomColor,
		seBorderLeft, seBorderLeftWidth, seBorderLeftStyle, seBorderLeftColor,

		seDisplay, seListStyle, seListImage, seListPos, seWhiteSpace,
		seFilter);
	...
	public
		property Elements[i: Integer]: THTMLElement read GetElements write SetElements; default;
		destructor Destroy; override;
	end;
  ...
  public
    constructor Create(AOwner: TTagObject); override;
    property Cells[ACol, ARow: Integer]: THTMLElement read GetCells write SetCells;
...
const
	HTMLAlignData: array[THTMLAlign] of string= ('Left', 'Right', 'Top', 'TextTop', 'Middle',
...
begin
	...
	begin
		...
		if s[i] = '"' then Breaket := not Breaket;
		if ((s[i] in BreakChars) or (i=Length(s))) and (T <> '') and (not Breaket) then
		begin
			case Stage of
			1: begin Result.TagName := Trim(T); Stage := 2; end;
			2: if not Breaket then Result.Properties.Add(Trim(T));
			end;
			t := '';
		end;
	end;
...
begin
	...
	for i := 0 to 90 do
	begin
		if CompareText(s, HTMLTags[i])=0 then
		begin
			Result := THTML4Tags(i);
			Exit;
		end;
...
constructor THTMLElement.CreateFromText(AOwner: TTagObject;
  ASource: String);
begin
	Self.Create(Aowner);
  fOrignal := ASource;
...
begin
	...
	begin
		...
		if ((s[i] in BreakChars) or (i=Length(s))) and (T <> '') and (not Breaket) then
	 	begin
    	case Stage of
			1: begin TagName := Trim(T); Stage := 2; end;
      2: if not Breaket then Properties.Add(Trim(T));
      end;
      t := '';
		end;
	end;
...
const
	fElementsName: array[TStyleSheetElement] of String = (
		...
		'display', 'list-style-type', 'list-style-image', 'list-style-position', 'white-space',
		'filter'
		);
var
	S, T, CSS: String;
	...
	begin
		...
		begin
			if P=fElementsName[i] then Elements[i] := Value;
		end;
	end;
...
begin
	...
	begin
		...
		'/': if i<>Length(CSS) then if Css[i+1]='*' then i := Instr(i, Css, '*/');
		...
		//add everything else
		else S := S + WideCharToString(@Css[i]);
		end;
		Inc(i);
	end;
	if PropertyName <> '' then
	...
	begin
		...
		'}':
			...
			begin
				...
				begin
					ClassLen := i-StartPos;
					Classes.Add(TStyleClass.CreateFromText(Owner, Copy(Css, StartPos, ClassLen)));
					StartPos := -1;
				end;
			end;
		end;
...
procedure THTMLTable.SetCells(ACol, ARow: Integer;
  const Value: THTMLElement);
begin
