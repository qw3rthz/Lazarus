...
type
	...
	TParseElementCompleteEvent = procedure (Sender: TObject; ElementType: TElementType;
...
//	TParseTagBeginEvent = procedure (Tag: TTagObject; var Continue: Boolean);
	...
	TParseErrorEvent = procedure (Sender: TObject; AErrorCode: Integer; AErrorMsg: String; var Continue: Boolean);
    ...
    procedure SetOnElementComplete(
      const Value: TParseElementCompleteEvent);
		procedure SetOnError(const Value: TParseErrorEvent);
		procedure DoError(ErrCode: Integer; ErrMsg: String; var Continue: Boolean);
		procedure DoComplete(ElementType: TElementType; Element: THTMLElement; var Continue: Boolean);
	protected
	...
	public
		...
		property OnElementComplete: TParseElementCompleteEvent read FOnElementComplete write SetOnElementComplete;
...
const
	...
	DefaultParents = 'HTML,P,A,FORM,FRAME,NOFRAME,TITLE,HEAD,STYLE,'+
		'TABLE,TR,UL,OL,OPTION,MENU,'+
		'DIV,SPAN,LAYER,OBJECT,APPLET,PRE';
var
	ParentTags: TStrings;
...
begin
    ...
    Inc(fPosition, Length(THTMLElement(fTags[i]).Text));  
		if TObject(fTags[i]) is TTagObject then
			if (CompareText(TTagObject(fTags[i]).TagName, fTagname) = 0)
      	or (fTagname='') then
      begin
...
var
	...
	IsScript, IsString, ISJSBlockComment, IsComment, IsStyle, IsCF, IsTag: Boolean;
	IgnoreChar: Boolean;
	TagState: TTagState;
	fContinue: Boolean;
begin
		...
		begin
				...
				begin
					...
					begin
						...
						begin
							...
							DoError(E_SYMBOL_INGORED, IntToStr(i) + ': "<" Symbol is ignored', fContinue);
					end
			...
			' ', #13, #10, #8, #9:
				begin
					...
					if IsScript and IsTag then IsTag := False;
					//If IsTag then check if is Begining a PropertyName or PropertyValue
					if (not IsString) and (IsTag) then
					begin
						if TagState = tsBegin then TagState := tsPropertyName;
						if TagState = tsPropertyValue then TagState := tsPropertyName;
					end;
					if ((Trim(S)='<!--') or (Trim(S)='<!---')) and IsScript then IsCF := True;
					...
					begin
						...
						fTags.Add(tag);
						t := '';
					end else if (AChar=#13) then
						if IsScript then if IsComment and (not ISJSBlockComment) then IsComment := False;
					s := '';
				end;
			'/':
				if IsScript then
				begin
					if Buffer[i-1]='/' then
					begin
						if not (IsComment or IsString or ISJSBlockComment) then IsComment := True;
					end
					else
					begin
			...
			'"', '''':
				...
				begin
					if Buffer[i-1] <> '\' then
						if StringChar = AChar then IsString := False;
				end
				else
				begin
			...
			begin
				T := T + AChar;
				S := S + AChar;
				if IsScript then Scripts := Scripts + AChar;
			end;
			IgnoreChar := False;
		end;

		if fContinue then goto ParseEnd;
...
ParseEnd:
	...
	if IsTag then LastState := LastState or bitTag;
	if IsScript then LastState := LastState or bitScript;
	if IsCF then LastState := LastState or bitCF;
	if IsComment then LastState := LastState or bitComment;
	if IsString then LastState := LastState or bitString;
...
procedure THTMLObject.SetOnElementComplete(
  const Value: TParseElementCompleteEvent);
begin
...
procedure THTMLObject.DoComplete(ElementType: TElementType;
	Element: THTMLElement; var Continue: Boolean);
begin
	if Assigned(FOnElementComplete) then FOnElementComplete(Self, ElementType, Element, Continue);
end;

procedure THTMLObject.DoError(ErrCode: Integer; ErrMsg: String; var Continue: Boolean);
begin
	if Assigned(FOnError) then FOnError(Self, ErrCode, ErrMsg, Continue);
end;
...
initialization
	...
	else ParentTags.CommaText := DefaultParents;
