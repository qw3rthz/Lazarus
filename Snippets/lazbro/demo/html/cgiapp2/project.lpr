...
uses
  ...
  begin
    ...
    Result := WriteRequestString('Details', itTextArea, pnDetails) and Result;
  end;
  ...
  begin
    WriteLn('<HTML>');
    WriteLn('<HEAD><TITLE>Custom CGI Application 2 - Demo</TITLE></HEAD>');
    Write('<BODY style=background-color:#eeeeee><FORM name=f method=get>');
    WritePageHeader;
    WriteTableBegin;
    try
