...
// http://wiki.portugal-a-programar.pt/dev_geral:pascal:delphi:snippet:parser
// http://www.portugal-a-programar.pt/topic/48331-expression-parser-em-delphi/page__st__60
{$ifdef FPC}
{$mode DELPHI}
{$endif}
...
function EvaluateConditional(Expression: ShortString): real;
var
   History:TStringList;
...
function EvaluateConditional(Expression: ShortString): real;
//Joaquim lloret, 2013
{Entrega el resultado de una expresión condicional de tipo
          ?<EXP1>;<EXP-T>;<EXP-F>
          ?<Arg1>=|>|<|!<Arg2>;<EXP-T>;<EXP-F>
<EXP1>: Expresión a evaluar, construida como <ARG1><OP><ARG2>. Puede usar los operadores:
= - Igual
> - Mayor que
< - Menor que
! - Distinto que

Si la condición es cierta devuelve EXP-T. Si es fals devuelve EXP-F}
var
  exp1, expt, expf: shortstring;
  p: integer;
begin
  p := pos(';', Expression);
  exp1 := MidStr(Expression, 2, p - 2);
  expt := MidStr(Expression, p + 1, PosEx(';', Expression, p + 1) - p - 1);
  expf := RightStr(Expression, length(Expression) - PosEx(';', Expression, p + 1));
  if Parse(exp1) = 0 then
    Result := Parse(expf)
  else
    Result := Parse(expt);
end;
